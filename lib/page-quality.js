...
const navigationError=e=>/execution context was destroyed|cannot find context|most likely because of a navigation|target closed/i.test(String(e?.message||e));
async function snapshot(page){return page.evaluate(()=>({title:document.title||'',text:(document.body?.innerText||'').replace(/\s+/g,' ').trim().slice(0,5000),elements:document.body?.querySelectorAll('*').length||0,links:document.querySelectorAll('a[href]').length,images:document.images.length,url:location.href}));}
export async function waitForUsablePage(page,{timeout=20000}={}){
 ...
 while(Date.now()-started<timeout){
  try{
   ...
   if(!challenge&&last.text.length>=120&&last.elements>=12){
    await page.waitForTimeout(750);
    try{const stable=await snapshot(page);if(stable.url===last.url)return stable;}catch(e){if(!navigationError(e))throw e;}
   }
  }catch(e){if(!navigationError(e))throw e;}
  await page.waitForTimeout(750).catch(()=>{});
 }
 try{return await snapshot(page);}catch(e){if(navigationError(e)){await page.waitForLoadState('domcontentloaded',{timeout:3000}).catch(()=>{});return await snapshot(page);}throw e;}
}
